import { ArrowLeft, BadgeCheck, ChevronRight, PenBox, User, X } from 'lucide-react'
...
export default function Rides() {
  ...
  const { rides, getAllRides, joinRide, checkIfUserIsPassenger } = useRideStore(); 

  const { infoFilled } = useSuggestionStore();
      ...
      <div className="flex flex-col py-10 gap-5">
        ...
        <div className="flex flex-col justify-center gap-10">
          ...
          <div className="flex items-center justify-between">
            <h1 className='font-bold text-xl'>Available Rides({rides.length})</h1>
          </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-4 w-full">
                    ...
                    <div className="flex flex-col justify-center gap-3 p-3">
                      <button disabled={checkIfUserIsPassenger()} className="btn btn-primary font-bold" onClick={()=>{
                        document.getElementById(`my_ride_confirm_modal_${index}`).showModal();
                      }}><User className="size-5"/>{ checkIfUserIsPassenger() ? "Booked" : "Book Now" }</button>
                    </div>
                  </div>
