...
import TrendingRideItemSkeleton from "../TrendingRideItemSkeleton/TrendingRideItemSkeleton";

export default function TrendingRides() {
  const { getTrendingRides, trendingRides, loading } = useRideAlertStore();
  useEffect(()=>{
    getTrendingRides();
  }, [])
  return (
    <div className="absolute top-0 right-0 h-full w-[20%] mr-5 overflow-y-scroll py-2 scrollbar">
      ...
      <div className="flex flex-col justify-center gap-3 py-5">
        {loading ? [...Array(10)].map((_, index)=>{
          return <TrendingRideItemSkeleton key={index} />
        }) : trendingRides.map((ride, index) => {
          return <TrendingRidesItem key={index} ride={ride} index={index} />;
          // return <TrendingRideItemSkeleton key={index} ride={ride} index={index} />
        })}
      </div>
    </div>
  );
