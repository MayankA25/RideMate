...
import LiveTrackingConfirmation from "../LiveTrackingConfirmation/LiveTrackingConfirmation";

export default function RideInfo() {
  const params = useParams();
  console.log("Ride Info Params: ", params.id);

  const { getRideInfo, selectedRide, checkIfUserIsPassenger, gettingRideInfo } =
    useRideStore();
      ...
      <div className="flex flex-col justify-between h-full gap-5">
        ...
        <div className="grid grid-cols-2">
            ...
            <div className="flex flex-col justify-center gap-7">
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col justify-center">
                  ...
                  {gettingRideInfo ? (
                    <span className="skeleton text-transparent mt-1">
                      Pickup Address Line 1 Lorem ipsum dolor sit amet
                      consectetur adipisicing elit.
                    </span>
                  ) : (
                    <span className="text-sm font-semibold text-white/50">
                      {!selectedRide?.pickup.addressLine1
                        ? ""
                        : `${selectedRide?.pickup.addressLine1},`}
                ...
                <div className="flex flex-col justify-center">
                  ...
                  {gettingRideInfo ? (
                    ...
                    <span className="font-semibold text-white/50">
                      {!selectedRide?.destination.addressLine1
                        ? ""
                        : `${selectedRide?.destination.addressLine1},`}
          ...
          <div className="grid grid-cols-1 justify-center pt-14 text-center lg:text-left">
            {gettingRideInfo ? (
              <h1 className="skeleton text-transparent">
            ...
            {gettingRideInfo ? (
              <h1 className="text-transparent skeleton my-1">0h 00min</h1>
            ) : (
        ...
        <div className="flex items-center justify-between py-10 px-5">
          <div className="flex items-center gap-3">
            {gettingRideInfo ? (
              <h1 className="text-transparent skeleton p-3">0</h1>
            ) : (
            ...
            <span
              className={`font-semibold text-2xl ${gettingRideInfo ? "skeleton text-transparent" : ""}`}
            >
          ...
          <div className="flex items-center">
            {gettingRideInfo ? (
              <h1 className="skeleton text-transparent text-3xl">Fare Here</h1>
            ) : (
        ...
        <div className="flex flex-col justify-center w-full gap-5 px-3">
          <h1
            className={`font-bold text-xl ${gettingRideInfo && "skeleton text-transparent py-2"}`}
          >
            Driver
          </h1>
          <div
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                {gettingRideInfo ? (
                  <h1 className="skeleton text-transparent text-2xl">
                    Driver Name
                  </h1>
                ) : (
                    ...
                    <BadgeCheck
                      className={`${
                        ((selectedRide?.driver.aadharCardStatus == "verified" &&
                          selectedRide?.driver.drivingLicenseStatus !=
                            "verified") ||
                          (selectedRide?.driver.aadharCardStatus !=
                            "verified" &&
                            selectedRide?.driver.drivingLicenseStatus ==
                              "verified")) &&
                        "text-yellow-300"
            ...
            {gettingRideInfo ? (
              <div className="flex items-center p-4 skeleton"></div>
            ) : (
              <div className="flex items-center">
                <ChevronRight />
              </div>
          ...
          <div className="flex items-center gap-4 ">
            <div className="flex items-center gap-8">
              <h1
                className={`font-bold text-md ${gettingRideInfo ? "skeleton text-transparent" : ""}`}
              >
                Car:{" "}
                <span className="font-bold">
                  {selectedRide?.carName} - {selectedRide?.carColor}
                </span>
              </h1>
            </div>
          </div>
          {checkIfUserIsPassenger(selectedRide) && (
            <div className={`flex items-center w-full`}>
              <button
                className={`py-3 font-bold text-sm text-center w-full hover:bg-base-200 transition-all duration-300 cursor-pointer rounded-full border ${gettingRideInfo ? "skeleton text-transparent" : "border-indigo-500"}`}
                onClick={() => {
        ...
        {!(gettingRideInfo || selectedRide?.passengers.length == 0) && (
        ...
        {selectedRide?.passengers.length != 0 && (
          <div className="flex flex-col justify-center px-3 py-3">
            <div className="flex flex-col justify-center gap-5">
              {
                <h1
                  className={`font-bold text-2xl ${gettingRideInfo && "skeleton text-transparent py-2"}`}
                >
        ...
        <div className="flex flex-col justify-center px-3 py-5 gap-8">
          <button className="flex gap-2 text-md text-indigo-300">
            {gettingRideInfo ? (
              <div className="size-8 skeleton text-transparent"></div>
            ) : (
              <Upload
                className="cursor-pointer size-5.5"
                onClick={(e) => {
                  if (gettingRideInfo) return;
        ...
        {!gettingRideInfo && user && user?._id != selectedRide?.driver?._id && (
          <div className="flex items-center justify-center sticky bottom-0 w-[100vw] transform -translate-x-[11%]">
            <button
              disabled={
                selectedRide?.passengers.length >=
                  selectedRide?.availableSeats ||
                !authenticated ||
                !checkIfUserIsPassenger(selectedRide)
              }
              ...
              onClick={() => {
                checkIfUserIsPassenger(selectedRide)
                  ? (selectedRide?.isLiveTrackingEnabled
                    ? null
                    : toast.error("Live Tracking Not Enabled Yet"))
                  : document
                      .getElementById(`my_ride_confirm_modal_${2}_${0}`)
                      .showModal();

                  checkIfUserIsPassenger(selectedRide) && navigate(`/map/${selectedRide?._id}`);
              }}
            >
              {selectedRide?.passengers.length >= selectedRide?.availableSeats
                ? "Full"
                : checkIfUserIsPassenger(selectedRide)
                  ? "Track Ride"
                  : "Book Now"}
        ...
        <LiveTrackingConfirmation rideId={params.id} />
        {!gettingRideInfo && user && user?._id == selectedRide?.driver?._id && (
          <div className="flex items-center justify-center sticky bottom-0 w-[100vw] transform -translate-x-[11%]">
            <button
              ...
              className="btn btn-primary w-full py-8 text-xl"
              onClick={() => {
                // toggleLiveTracking(params.id);
                document
                  .getElementById("my_live_tracking_confirmation_modal")
                  .showModal();
              }}
            >
              {selectedRide?.isLiveTrackingEnabled
