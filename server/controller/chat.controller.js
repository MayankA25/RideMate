...
export const sendMessage = async (req, res) => {
  ...
  try {
    ...
    const newGroupMessage = await (
      await (await savedMessage.populate("sender")).populate("group")
    ).populate("parentId");
...
export const updateMessage = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ msg: "Message has beem updated", message: updatedMessage });
  } catch (e) {
    console.log(e);
    return res.status(500).json({ msg: "Internal Server Error" });
  }
...
export const deleteMessage = async (req, res) => {
  ...
  try {
    ...
    if (foundMessage.sender._id != userId)
      return res.status(401).json({ msg: "Unauthorized Deletion" });
    ...
    console.log("Updated Message: ", updatedMessage);
    io.to(rideId).emit("deletedGroupMessageId", {
      messageId: messageId,
...
export const replyToMessage = async (req, res) => {
  ...
  try {
    const foundMessage = await Message.findById(parentId)
      .populate("sender")
      .populate("group")
      .populate("parentId");
    ...
    const newReply = await (
      await (await savedReply.populate("sender")).populate("group")
    ).populate("parentId");
